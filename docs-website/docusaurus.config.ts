import type * as Preset from '@docusaurus/preset-classic';
import type { Config } from '@docusaurus/types';
import { themes as prismThemes } from 'prism-react-renderer';

const config: Config = {
  title: 'Skip-The-Build Docs',
  favicon: 'img/favicon.ico',

  url: 'https://spautz.github.io',
  baseUrl: '/skip-the-build/',

  // GitHub pages deployment config.
  organizationName: 'spautz',
  projectName: 'skip-the-build',

  onBrokenLinks: 'throw',

  // Even if you don't use internationalization, you can use this field to set
  // useful metadata like html lang. For example, if your site is Chinese, you
  // may want to replace "en" with "zh-Hans".
  i18n: {
    defaultLocale: 'en',
    locales: ['en'],
  },

  presets: [
    [
      'classic',
      {
        docs: {
          routeBasePath: '/',
          sidebarPath: './sidebars.ts',
        },
        blog: {
          showReadingTime: true,
        },
        theme: {
          customCss: './src/css/custom.css',
        },
      } satisfies Preset.Options,
    ],
  ],

  themeConfig: {
    // Replace with your project's social card
    image: 'img/docusaurus-social-card.jpg',
    announcementBar: {
      id: 'global-banner-2026-03-03',
      content:
        '<br/><p>These docs have been autogenerated, and have not yet been reviewed by a human.</p><p>Please verify anything you read here, or wait for the next revision.</p>',
      backgroundColor: '#fff4d1',
      textColor: '#1f2937',
      isCloseable: false,
    },
    navbar: {
      title: "spautz's Skip-The-Build Docs\n",
      // logo: {
      //   alt: 'My Site Logo',
      //   src: 'img/logo.svg',
      // },
      items: [
        {
          type: 'docSidebar',
          sidebarId: 'mainSidebar',
          position: 'left',
          label: 'Home',
        },
        // { to: '/blog', label: 'Blog', position: 'left' },
        // { to: '/demo', label: 'Demo', position: 'left' },
        {
          href: 'https://github.com/spautz/skip-the-build',
          label: 'GitHub',
          position: 'right',
        },
      ],
    },
    footer: {
      style: 'dark',
      // links: [
      //   {
      //     title: 'Docs',
      //     items: [
      //       {
      //         label: 'Home',
      //         to: '/',
      //       },
      //       {
      //         label: 'Demo',
      //         to: '/demo',
      //       },
      //     ],
      //   },
      //   {
      //     title: 'More',
      //     items: [
      //       {
      //         label: 'Blog',
      //         to: '/blog',
      //       },
      //       {
      //         label: 'GitHub',
      //         href: 'https://github.com/spautz/skip-the-build',
      //       },
      //     ],
      //   },
      // ],
      copyright: 'Built with Docusaurus',
    },
    markdown: {
      hooks: {
        onBrokenMarkdownLinks: 'throw',
      },
    },
    prism: {
      theme: prismThemes.github,
      darkTheme: prismThemes.dracula,
    },
  } satisfies Preset.ThemeConfig,
};

export default config;
